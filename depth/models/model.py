...
import torch
import random
from transformers import AutoImageProcessor, AutoModelForImageClassification, SegformerImageProcessor, SegformerForSemanticSegmentation
...
from edadepth.models import UNetWrapper, BEiTAdapter

from transformers import logging, BeitFeatureExtractor, Swin2SRForImageSuperResolution, BeitForSemanticSegmentation
...
class CIDE(nn.Module):
    def __init__(self, args, emb_dim):
        super().__init__()
        self.args = args
        self.vit_processor = BeitFeatureExtractor.from_pretrained("microsoft/beit-base-finetuned-ade-640-640", resume_download=True)
        self.vit_model = BeitForSemanticSegmentation.from_pretrained("microsoft/beit-base-finetuned-ade-640-640", resume_download=True)
        ...
        self.fc = nn.Sequential(
            nn.Linear(150, 120),
            nn.GELU(),
            nn.Linear(120, args.no_of_classes)
        )
        self.dim = emb_dim
        self.m = nn.Softmax(dim=1)
        
        self.embeddings = nn.Parameter(torch.randn(self.args.no_of_classes, self.dim))
        self.embedding_adapter = BEiTAdapter(emb_dim=self.dim)
        
        self.gamma = nn.Parameter(torch.ones(self.dim) * 1e-4)
    def pad_to_make_sr(self, x):
        # Initialize the super-resolution processor and model
        sr_processor = AutoImageProcessor.from_pretrained("caidas/swin2SR-classical-sr-x2-64")
        sr_model = Swin2SRForImageSuperResolution.from_pretrained("caidas/swin2SR-classical-sr-x2-64").to(x.device)

        # Rescale pixel values from [-1, 1] to [0, 255] and convert to integer type
        y = 255 * ((x + 1) / 2)
        y = torch.permute(y, (0, 2, 3, 1))  # Permute dimensions to (batch_size, height, width, channels)
        y = y.to(torch.int)  # Convert to integer type

        # Prepare inputs for the super-resolution model
        sr_inputs = sr_processor(images=y.cpu().numpy(), return_tensors="pt").to(x.device)

        # Apply the super-resolution model
        with torch.no_grad():
            sr_outputs = sr_model(**sr_inputs)

        # Extract the super-resolved image
        sr_image = sr_outputs.reconstruction

        # Permute dimensions back to (batch_size, channels, height, width)
        sr_image = torch.permute(sr_image, (0, 3, 2, 1))

        # Clip values to the range [0, 255] and convert to uint8
        sr_image = sr_image.clamp(0, 255).to(torch.uint8)

        return sr_image

    def forward(self, x):
        # Pad to make super resolution
        y = self.pad_to_make_sr(x)
        # use torch.no_grad() to prevent gradient flow through the ViT since it is kept frozen
        with torch.no_grad():
            inputs = self.vit_processor(images=y.cpu().numpy(),return_tensors="pt").to(x.device)
            vit_outputs = self.vit_model(**inputs)
            vit_logits = vit_outputs.logits
            vit_logits = vit_logits[:,:,0,0]
